...
def test_group_flow_end_to_end_over_http():
    """The whole group loop works over the customer + staff HTTP endpoints.

    Leader POSTs start → member POSTs join → staff POSTs confirm-group → the
    response carries the single leader voucher and the session reads COMPLETED.
    """
    business = make_business()
    staff = make_staff(business)
    campaign = _group_campaign(business, status=Campaign.Status.ACTIVE, group_size=2)
    leader = make_customer("871")
    member = make_customer("872")

    start = _auth(leader).post(
        f"/api/customer/campaigns/{campaign.id}/group/start/"
    )
    assert start.status_code == 201
    invite_token = start.data["data"]["invite_token"]
    session_id = start.data["data"]["id"]

    join = _auth(member).post(
        "/api/customer/campaign-groups/join/",
        {"invite_token": invite_token},
        format="json",
    )
    assert join.status_code == 201

    confirm = _auth(staff.user).post(
        "/api/staff/campaigns/confirm-group/",
        {"group_session_id": session_id},
        format="json",
    )
    assert confirm.status_code == 200
    assert confirm.data["data"]["voucher"]["status"] == "active"
    assert confirm.data["data"]["session"]["status"] == "completed"
    assert (
        CampaignRewardVoucher.objects.filter(campaign=campaign, customer=leader).count()
        == 1
    )
