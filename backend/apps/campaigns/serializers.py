...
class CampaignSerializer(serializers.ModelSerializer):
    ...
    def get_type_stats(self, obj: Campaign) -> dict:
        ...
        if obj.campaign_type == Campaign.CampaignType.GROUP:
            return {
                "stat_a": {
                    ...
                    "value": getattr(obj, "_groups", 0) or 0,
                },
                "stat_b": {
                    "label": "Customers joined",
                    "value": getattr(obj, "_members", 0) or 0,
                },
        ...
        return {
            ...
            "stat_b": {"label": "Redeemed", "value": redeemed},
            "stat_c": {
                "label": "Close to reward",
                "value": getattr(obj, "_close", 0) or 0,
            },
        }
...
class CampaignDetailSerializer(CampaignSerializer):
    ...
    business_category = serializers.CharField(
        source="business.category", read_only=True
    )
...
class CampaignRewardVoucherSerializer(serializers.ModelSerializer):
    ...
    item_selection = serializers.CharField(
        source="reward.item_selection", read_only=True, default=None
    )
...
class EligibleCampaignViewSerializer(serializers.Serializer):
    ...
    Campaign rows expose eligibility and visit-count progress only.
    """
...
class ProgressResultSerializer(serializers.Serializer):
    ...
    ``voucher`` is populated only when the visit finishes a cycle.
    """
...
class UnifiedScanResultSerializer(serializers.Serializer):
    ...
    def get_skipped_campaigns(self, obj) -> list:
        return [
            {
                "campaign_id": str(sc.campaign_id),
                "name": sc.name,
