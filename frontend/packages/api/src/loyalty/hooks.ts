...
export const useUpdateLoyaltyProgram = (id: string) => { const qc = useQueryClient(); return useMutation({ mutationFn: (input: Partial<LoyaltyProgramInput>) => loyaltyApi.updateProgram(id, input), onSuccess: () => { qc.invalidateQueries({ queryKey: loyaltyKeys.ownerProgram(id) }); qc.invalidateQueries({ queryKey: loyaltyKeys.ownerPrograms }); } }); };
